...
internal sealed class RedeemableVisualizerClient : IDisposable
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var packageBaseUri = packageUri is null
            ? manifestAssetBaseUri
            : GetAssetBaseUri(packageUri);
    ...
    private async Task<string> ResolveModuleJsonAsync(
        JsonElement payload,
        Uri baseUri,
        CancellationToken cancellationToken)
    {
        ...
        return await DownloadTextAsync(ResolveHttpsUri(baseUri, moduleUrl, "module URL"), MaxModuleBytes, cancellationToken);
    }

    private async Task<byte[]> ResolveBinaryAsync(
        JsonElement payload,
        Uri baseUri,
        CancellationToken cancellationToken)
    {
        ...
        return await DownloadBytesAsync(ResolveHttpsUri(baseUri, binaryUrl, "binary URL"), MaxBinaryBytes, cancellationToken);
    }
    ...
    private async Task<JsonElement> DownloadJsonAsync(
        Uri uri,
        int maxBytes,
        CancellationToken cancellationToken)
    {
        var text = await DownloadTextAsync(uri, maxBytes, cancellationToken);
    ...
    private async Task<string> DownloadTextAsync(Uri uri, int maxBytes, CancellationToken cancellationToken)
    {
        var bytes = await DownloadBytesAsync(uri, maxBytes, cancellationToken);
        return System.Text.Encoding.UTF8.GetString(bytes);
    }

    private async Task<byte[]> DownloadBytesAsync(Uri uri, int maxBytes, CancellationToken cancellationToken)
    {
        EnsureHttps(uri);
    ...
    private static IEnumerable<Uri> GetManifestCandidates(Uri manifestUri)
    {
        ...
        var withoutTrailingSlash = manifestUri.ToString().TrimEnd('/');
        if (!withoutTrailingSlash.EndsWith(".json", StringComparison.OrdinalIgnoreCase))
        {
            yield return new Uri($"{withoutTrailingSlash}.json");
            yield return new Uri($"{withoutTrailingSlash}/manifest.json");
            yield return new Uri($"{withoutTrailingSlash}/index.json");
        }
    }
        ...
        JsonElement Payload)
    {
        ...
        private static bool EntryContainsKey(JsonElement entry, string normalizedRedeemKey)
        {
            ...
            {
                if (FirstString(entry, propertyName) is { } key &&
                    KeyMatches(key, normalizedRedeemKey))
                {
                    return true;
                }
            }
            ...
            foreach (var propertyName in new[] { "redeemKeys", "keys" })
            {
                if (!TryGetArray(entry, propertyName, out var keys))
                    continue;
