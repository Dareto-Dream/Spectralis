...
internal sealed class SharedPlayCacheStore
{
    ...
    public Task<SharedPlayPackage> CreateOrGetPackageAsync(AudioTrackInfo track, CancellationToken cancellationToken) =>
    ...
    private SharedPlayPackage CreateOrGetPackage(AudioTrackInfo track, CancellationToken cancellationToken)
    {
        EnsureCacheRoot();
        CleanupExpired();
    ...
    private static SharedPlayTrackDescriptor CreateTrackDescriptor(AudioTrackInfo track) =>
        new(
            track.DisplayName,
            track.Artist,
            track.Album,
            ...
            BuildSharedPlayLyrics(track.Lyrics));
    ...
    private static AudioTrackInfo BuildTrackInfo(string path)
    {
        var metadata = AudioMetadataReader.Read(path);
        var fallbackDisplayName = Path.GetFileNameWithoutExtension(path);
        var displayName = FirstNonEmpty(metadata.Title, fallbackDisplayName, Path.GetFileName(path)) ?? "Queued track";
        ...
        try
        {
            ...
            bitsPerSample = Math.Max(0, file.Properties.BitsPerSample);
        ...
        return new AudioTrackInfo(
            ...
            string.IsNullOrWhiteSpace(formatName) ? "Audio" : formatName,
            channels,
            sampleRate,
            bitsPerSample,
            duration);
