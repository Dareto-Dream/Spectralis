...
internal static class SunoClipResolver
{
    ...
    public static async Task<SunoClipInfo> ResolveAsync(string input, CancellationToken cancellationToken)
    {
        ...
        foreach (var pageUrl in BuildPublicPageUrls(clipId, input))
        {
            ...
            {
                // Public page metadata is best-effort. The CDN URL can still stream
                // when the caller supplied a valid UUID.
            }
        }
...
internal static class SunoLyricsBuilder
{
    ...
    public static LyricsDocument? BuildDescription(string? prompt)
    {
        var lines = NormalizePromptLines(prompt);
        if (lines.Count == 0)
            return null;

        var lyricLines = lines.Select(static (line, i) => new LyricsLine(i, line));
        return new LyricsDocument(lyricLines, sourceLabel: "Suno lyrics", isDescription: true);
    }

    private static List<string> NormalizePromptLines(string? prompt)
    {
        ...
        foreach (var rawLine in prompt.Replace("\r\n", "\n", StringComparison.Ordinal).Replace('\r', '\n').Split('\n'))
        {
            ...
            {
                var bracketContent = line[1..^1].Trim();
                if (LooksLikeSectionHeader(bracketContent))
                {
    ...
    private static bool LooksLikeSectionHeader(string value)
    {
        if (string.IsNullOrWhiteSpace(value))
        {
            return false;
        ...
        return SectionTokens.Any(token => lower.Contains(token, StringComparison.Ordinal));
    }
}
