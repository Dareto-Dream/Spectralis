...
internal sealed class ArtistStat
{
    public string Artist { get; init; } = "";
    public int    Plays  { get; set; }
}

internal sealed class TrackStat
{
...
internal sealed class ListeningStats
{
    ...
    public static ListeningStats Compute(IList<ScrobbleRecord> history, DateTime since)
    {
        ...
        var tracks = filtered
            ...
            .GroupBy(r => (r.Title.ToUpperInvariant(), r.Artist.ToUpperInvariant()))
    ...
    private static (int Current, int Longest) ComputeStreaks(IList<ScrobbleRecord> history)
    {
        ...
        if (activeDays[0] == today || activeDays[0] == today.AddDays(-1))
        {
            ...
            {
                if (day == expected) { current++; expected = expected.AddDays(-1); }
                else break;
            }
        }
