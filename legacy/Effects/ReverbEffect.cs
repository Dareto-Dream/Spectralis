...
internal sealed class ReverbEffect : IAudioEffect
{
    ...
    private sealed class ReverbSampleProvider : ISampleProvider
    {
        ...
        public int Read(float[] buffer, int offset, int count)
        {
            ...
            for (var i = 0; i < read; i += channels)
            {
                // Mix to mono input
                var input = 0f;
                for (var c = 0; c < channels; c++) input += buffer[offset + i + c];
                input /= channels;
                input *= 0.015f;  // scale down to avoid feedback clipping
                ...
                for (var cb = 0; cb < _combBuffers.Length; cb++)
                {
                    ...
                    var @out = buf[pos];
                    _combFiltered[cb] = @out * (1f - damp) + _combFiltered[cb] * damp;
