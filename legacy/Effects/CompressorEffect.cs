...
internal sealed class CompressorEffect : IAudioEffect
{
    ...
    private sealed class CompressorSampleProvider : ISampleProvider
    {
        ...
        public int Read(float[] buffer, int offset, int count)
        {
            ...
            {
                // Compute RMS-like level over this frame (just use peak of this sample for simplicity)
                var peak = 0f;
                ...
                if (levelDb > threshold)
                    targetGainDb = threshold + (levelDb - threshold) / ratio - levelDb;
                else
                    targetGainDb = 0f;

                // Smooth with attack/release
                if (targetGainDb < _gainDb)
                    _gainDb = attackCoeff  * _gainDb + (1 - attackCoeff)  * targetGainDb;
                else
                    _gainDb = releaseCoeff * _gainDb + (1 - releaseCoeff) * targetGainDb;
