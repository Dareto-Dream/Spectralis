...
ships one track with one optional experience, a `.spectral` capsule ships a whole album with
an interactive HTML "world" the creator fully controls.

Examples of what the world can be: a Super Mario World-style level map where each song is a
level, an interactive liner notes page, a branching narrative, whatever the creator wants to
build honestly. If the creator ships no world, the player just falls back to a plain tracklist.
...
`%LocalAppData%\Spectralis\trusted-creators.json` — one file, both formats, so don't go looking
for a seperate album trust store, there isn't one. A creator trusted for a single-track capsule
is not automatically trusted for an album capsule though — both formats intersect against the
CDN key's `allowedCapabilities` independently.
...
CORS headaches, no base-64 data URI workarounds, build it like a normal website.
...
### Minimal Album World Structure (the "I just want it to work" layout)
...
- Don't keep expensive animation loops running when `frame.active === false` (player paused
  or stopped) — your listener's battery and my inbox will both thank you. Use `onPlaybackFrame`
  to detect the inactive state and pause rendering.
- `onReady` may fire up to 200 ms after the page finishes loading. Design the page to show a
  sensible loading state before `onReady` arrives, don't just show a blank white flash.
