...
services. It's a contract for the hosted services, not for the player.
...
pipeline. Do not move this route, seriously.
...
capsule. It's also re-fetched periodically to check for revocations.
...
Five minutes is short enough to make revocations meaningful within a session, without hammering
the CDN every time someone opens a capsule.
...
Graph metadata for chat previews. It can also serve as the Discord Activity URL target, so don't
seperate the two paths without checking that first.
