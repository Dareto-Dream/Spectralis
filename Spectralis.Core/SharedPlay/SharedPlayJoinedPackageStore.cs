...
/// <summary>
/// Local cache for audio downloaded while joining someone else's Shared Play room as a
/// listener. Ported from the legacy WinForms `SharedPlayJoinedPackageStore` — same layout,
/// same 7-day retention, same extraction logic, just public (the App layer needs it) and typed
/// against <see cref="SharedPlayJoinedSession"/> instead of the legacy remote-session type.
/// </summary>
public sealed class SharedPlayJoinedPackageStore
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var sessionDirectory = GetSessionDirectory(session.RoomCode, session.TrackId);
    ...
    private string GetSessionDirectory(string roomCode, string? trackId)
    {
        var safeRoomCode = NormalizeSegment(roomCode);
        if (string.IsNullOrWhiteSpace(safeRoomCode))
            safeRoomCode = Guid.NewGuid().ToString("N");
        var safeTrackId = NormalizeSegment(trackId ?? "");

        var sessionDirectory = string.IsNullOrWhiteSpace(safeTrackId)
            ? Path.GetFullPath(Path.Combine(_cacheRoot, safeRoomCode))
            : Path.GetFullPath(Path.Combine(_cacheRoot, safeRoomCode, safeTrackId));
    ...
    private void EnsureCacheRoot()
    {
        ...
        try
        {
            File.SetAttributes(_cacheRoot, File.GetAttributes(_cacheRoot) | FileAttributes.Hidden);
        }
        catch
        {
        }
    ...
    {
        var normalizedRoot = _cacheRoot.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)
