...
public static class ObsOverlayHtml
{
    public const string Template = """
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        // In a regular browser window.obsstudio is absent — add a dark preview background
        // so widgets are visible when testing outside OBS.
        const inObs=!!window.obsstudio;
        if(!inObs){
          document.body.style.background='#111';
          const badge=document.createElement('div');
          badge.textContent='BROWSER PREVIEW';
          badge.style.cssText='position:fixed;top:10px;right:12px;z-index:9999;'
            +'font-size:10px;font-weight:700;letter-spacing:.1em;color:rgba(255,255,255,.3);'
        ...
        function buildNowPlaying(idx){
          ...
          if(def.showArt!==false){
            ...
            img.src=ART_PH;
            img.onerror=()=>{img.src=ART_PH;img.onerror=null;};
            el.appendChild(img);
          }
        ...
        // Only called on first attach and on resize â€” NOT every frame
        ...
        function applyNowPlaying(idx){
          ...
          if(titleEl)  titleEl.textContent =state.track?.title||"No track";
          ...
          if(artEl){
            if(av&&av!==widgets[idx].artVer){
              widgets[idx].artVer=av;
              artEl.onerror=()=>{artEl.src=ART_PH;artEl.onerror=null;};
              artEl.src=BASE+"/assets/artwork?v="+encodeURIComponent(av);
            } else if(!av&&widgets[idx].artVer){
              widgets[idx].artVer="";
              artEl.src=ART_PH;
            }
          }
        ...
        function applyQueue(el,def){
          ...
          (state.queue||[]).slice(0,max).forEach(q=>{
            ...
            d.textContent=(q.artist?q.artist+" â€” ":"")+q.title;
            list.appendChild(d);
          });
        }
        ...
        // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // VISUALIZER DRAW LOOP â€” driven by requestAnimationFrame
        // (OBS Browser sources use rAF tied to the source frame rate)
        // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        const TAU=Math.PI*2;
        let spinAngle=0;

        // Visualizer data only changes once per server push (~10/s); the canvas redraws
        // every rAF tick (~60/s). Without smoothing, bars visibly snap to the new value
        // each push instead of animating, looking choppy/low-framerate. Lerp the displayed
        // levels toward the latest pushed values each frame so motion stays smooth between pushes.
        ...
        function drawVizLoop(){
          const targetLv=state.visualizer?.levels||[];
          const targetRms=state.visualizer?.rms||0;
          const targetPeak=state.visualizer?.peak||0;

          if(smoothLevels.length!==targetLv.length) smoothLevels=targetLv.slice();
          for(let i=0;i<targetLv.length;i++){
            smoothLevels[i]+=(targetLv[i]-smoothLevels[i])*SMOOTHING;
          }
          smoothRms+=(targetRms-smoothRms)*SMOOTHING;
          smoothPeak+=(targetPeak-smoothPeak)*SMOOTHING;

          const lv=smoothLevels, rms=smoothRms, peak=smoothPeak;
          const themeAcc=state.theme?.accent||"#F59E0B";

          for(let i=0;i<widgets.length;i++){
            const w=widgets[i];
            if(!w.canvas||!w.ctx||w.def.type!=="visualizer") continue;
            // Use cached canvas dimensions â€” no getBoundingClientRect here
        ...
        // LED Meter â€” segmented stereo bars with peak hold
        ...
        // Vectorscope â€” Lissajous stereo-field plot
        ...
        // Spectrogram â€” scrolling waterfall using two-canvas GPU swap (no getImageData)
        ...
        // Bounce Bars â€” spectrum bars with gravity/physics
        function drawBounceBars(w,cw,ch,lv,int_,acc){
          ...
          for(let i=0;i<lv.length;i++){
            ...
            // Single solid fill â€” much faster than per-bar gradient
        ...
        // Circular EQ â€” 360Â° spectrum bars radiating from center
        ...
        // Block Grid â€” classic LED frequency grid
        ...
        function applyData(s){
          ...
          // Layout seq change detection â€” re-fetch and rebuild widget tree
