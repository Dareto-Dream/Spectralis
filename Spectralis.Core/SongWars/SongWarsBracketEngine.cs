...
public static class SongWarsBracketEngine
{
        ...
        int? randomSeed = null)
    {
        ...
        if (judges.Count is < 1 or > 5)
            throw new ArgumentException("Song Wars requires 1-5 judges.", nameof(judges));
    ...
    public static SongWarsMatch? GetCurrentMatch(SongWarsTournament tournament) =>
        string.IsNullOrWhiteSpace(tournament.CurrentMatchId)
            ? null
            : tournament.Matches.FirstOrDefault(m => string.Equals(m.MatchId, tournament.CurrentMatchId, StringComparison.OrdinalIgnoreCase));
    ...
    {
        var match = tournament.Matches.FirstOrDefault(m => string.Equals(m.MatchId, matchId, StringComparison.OrdinalIgnoreCase))
        ...
        if (outcome == SongWarsOutcome.Skip)
        {
            ...
            AppendAudit(tournament, "match-skipped", "Match skipped and requeued.", match.MatchId);
            SetCurrentToNextPending(tournament);
            Touch(tournament);
            return match;
        }
        ...
        AppendAudit(tournament, "match-complete", $"{match.RoundId} completed with {outcome}.", match.MatchId);
    ...
    internal static List<int> GenerateSeedOrder(int count)
    {
        ...
        while (order.Count < count)
        {
            ...
            foreach (var seed in order) { next.Add(seed); next.Add(nextSize + 1 - seed); }
            order = next;
        }
    ...
    {
        if (tournament.Matches.Any(IsOpenMatch)) return;
        ...
        if (zeroLoss.Count == 1 && oneLoss.Count == 1) { AddGrandFinalsMatch(tournament, zeroLoss[0], oneLoss[0], reset: false); return; }
        if (zeroLoss.Count == 0 && oneLoss.Count == 2) { AddGrandFinalsMatch(tournament, oneLoss[0], oneLoss[1], reset: true); return; }
        if (oneLoss.Count > 1 && lastCompleted?.Bracket == SongWarsBracket.Winners) { AddRoundMatches(tournament, SongWarsBracket.Losers, oneLoss); return; }
        if (zeroLoss.Count > 1) { AddRoundMatches(tournament, SongWarsBracket.Winners, zeroLoss); return; }
        if (oneLoss.Count > 1) AddRoundMatches(tournament, SongWarsBracket.Losers, oneLoss);
    }

    private static void AddRoundMatches(SongWarsTournament tournament, SongWarsBracket bracket, IReadOnlyList<SongWarsSubmission> participants)
    {
        ...
        if (startIndex == 1)
            AppendAudit(tournament, "round-bye", $"{participants[0].DisplayTitle} received a {roundId} bye.");
    ...
    private static void AddGrandFinalsMatch(SongWarsTournament tournament, SongWarsSubmission slotA, SongWarsSubmission slotB, bool reset)
    {
    ...
    private static SongWarsMatch AddMatch(SongWarsTournament tournament, SongWarsBracket bracket, int roundIndex, string roundId, string slotAId, string slotBId)
    {
    ...
    private static void RequeueSkippedMatch(SongWarsTournament tournament, SongWarsMatch skipped)
    {
        ...
        var insertAfter = tournament.MatchOrder
            ...
            .Where(item => tournament.Matches.FirstOrDefault(m => m.MatchId == item.id)?.RoundId == skipped.RoundId)
    ...
    private static (string WinnerId, string LoserId, bool DirectElimination) ResolveResult(SongWarsMatch match, SongWarsOutcome outcome)
    {
    ...
    private static void ApplySubmissionResult(SongWarsTournament tournament, SongWarsMatch match, string winnerId, string loserId, bool directElimination)
    {
        var winner = FindSubmission(tournament, winnerId);
        var loser = FindSubmission(tournament, loserId);

        winner.Status = winner.Losses == 0 ? SongWarsSubmissionStatus.Active : SongWarsSubmissionStatus.LosersBracket;
        ...
        loser.Losses++;
        loser.Status = loser.Losses >= 2 ? SongWarsSubmissionStatus.Eliminated : SongWarsSubmissionStatus.LosersBracket;
    ...
    private static int NextRoundIndex(SongWarsTournament tournament, SongWarsBracket bracket) =>
        tournament.Matches.Where(m => m.Bracket == bracket).Select(m => m.RoundIndex).DefaultIfEmpty(0).Max() + 1;

    private static string BuildRoundId(SongWarsBracket bracket, int roundIndex) =>
        bracket switch
        {
            SongWarsBracket.Winners => roundIndex switch { 4 => "winners-semis", 5 => "winners-finals", _ => $"winners-r{roundIndex}" },
            SongWarsBracket.Losers => roundIndex switch { 4 => "losers-semis", 5 => "losers-finals", _ => $"losers-r{roundIndex}" },
    ...
    private static bool IsOpenMatch(SongWarsMatch match) =>
        match.Result == SongWarsOutcome.Pending && match.Phase != SongWarsMatchPhase.Skipped;
    ...
    private static List<SongWarsSubmission> NormalizeSubmissions(IReadOnlyList<SongWarsSubmission> submissions, int? randomSeed)
    {
