...
public sealed class PlayQueue
{
    ...
    private int NextIndex()
    {
        ...
        if (_shuffle)
        {
            ...
            {
                // Repeat-all wrap: fresh unpinned order (legacy behavior).
                if (Repeat == RepeatMode.All) { RebuildShuffle(pinCurrent: false); return _shuffleOrder![0]; }
                return -1;
            }
    ...
    /// <summary>Returns the path of the next track without advancing the queue pointer.</summary>
    ...
    // Playlist load/save hooks return with the playlist system port.

    private void RebuildShuffle(bool pinCurrent = true)
    {
        _shuffleOrder = Enumerable.Range(0, _items.Count).OrderBy(_ => _random.Next()).ToList();

        // Pin the playing track to the front so a full pass visits every other
        // item exactly once (the WinForms queue left it mid-order, silently
        // skipping everything shuffled before it).
        if (pinCurrent && _currentIndex >= 0 && _shuffleOrder.Remove(_currentIndex))
        {
