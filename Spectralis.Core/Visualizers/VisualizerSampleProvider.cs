...
public sealed class VisualizerSampleProvider : ISampleProvider
{
    ...
    public VisualizerFrame GetFrame(bool includeSpectrogram = false, bool includeRawFft = false)
    {
        lock (syncRoot)
        {
            ...
            var spectrogramCopy = Array.Empty<float[]>();
            if (includeSpectrogram)
            {
                spectrogramCopy = new float[SpectrogramHistoryLength][];
                for (var i = 0; i < SpectrogramHistoryLength; i++)
                ...
                RmsLevel: rmsLevel)
            {
                WaveformL = waveformL,
                WaveformR = waveformR,
                RawFftBins = includeRawFft || includeSpectrogram
                    ? (float[])rawFftBins.Clone()
                    : Array.Empty<float>(),
                ...
                SpectrogramNewestIndex = includeSpectrogram ? spectrogramNewestIndex : 0,
            };
        }
    }
