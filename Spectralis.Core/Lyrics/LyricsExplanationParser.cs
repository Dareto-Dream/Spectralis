...
public static class LyricsExplanationParser
{
    ...
    public static string TimestampKey(double startTimeSeconds) =>
        FormatTimestamp(TimeSpan.FromSeconds(startTimeSeconds));

    public static void Save(string annotationPath, IReadOnlyDictionary<string, string> annotations)
    {
        var writer = new System.Text.StringBuilder();
        writer.Append('{');
        var first = true;
        foreach (var (key, value) in annotations.OrderBy(static kvp => kvp.Key, StringComparer.Ordinal))
        {
            ...
            writer.AppendLine();
            writer.Append("  ");
            writer.Append(System.Text.Json.JsonSerializer.Serialize(key));
            writer.Append(": ");
            writer.Append(System.Text.Json.JsonSerializer.Serialize(value));
        }
        if (!first) writer.AppendLine();
        writer.Append('}');
        File.WriteAllText(annotationPath, writer.ToString(), System.Text.Encoding.UTF8);
    }
