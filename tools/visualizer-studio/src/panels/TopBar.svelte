<script lang="ts">
  ...
  import { buildVisualizerHtml } from '../export/buildVisualizerHtml';
  ...
  async function doExport() {
    ...
    exporting = true;
    try {
      // buildVisualizerHtml is a plain (non-Svelte) module so it stays testable
      // without the runes machinery — structuredClone inside it can't clone a
      // live $state proxy directly, so snapshot to a plain object here first.
      const html = buildVisualizerHtml($state.snapshot(store.project) as typeof store.project);
      downloadText(`${store.project.meta.slug}_visualizer.html`, html);
      toast.push('success', `Exported ${store.project.meta.slug}_visualizer.html`);
    } finally {
      exporting = false;
    }
  }
